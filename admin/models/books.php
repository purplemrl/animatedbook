 ...
 * @subpackage  Models
 */
class Davidix_animatedbookModelBooks extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.publish_down', 'publish_down','ordering', 'state', 'type', 'name', 'author', 'description', 'short_description', 'ribbon_text', 'ribbon_color', 'cover_top_color', 'cover_buttom_color', 'cover_image', 'book_direction', 'link1', 'link1_src', 'link2', 'link2_src'
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__books AS a');
		...
		if (!empty($search))
		{
			...
			{
				$search = $db->quote('%' . $db->escape(substr($search, strlen('type:')), true) . '%');
			...
			else
			{
				...
				$query->where('a.type LIKE' . $s . ' OR a.name LIKE' . $s . ' OR a.author LIKE' . $s . ' OR a.description LIKE' . $s . ' OR a.short_description LIKE' . $s . ' OR a.ribbon_text LIKE' . $s . ' OR a.ribbon_color LIKE' . $s . ' OR a.cover_top_color LIKE' . $s . ' OR a.cover_buttom_color LIKE' . $s . ' OR a.cover_image LIKE' . $s . ' OR a.book_direction LIKE' . $s . ' OR a.link1 LIKE' . $s . ' OR a.link1_src LIKE' . $s . ' OR a.link2 LIKE' . $s . ' OR a.link2_src LIKE' . $s );
			}
		}
		...
		$sort = $this->getState('list.ordering', 'type');
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__books AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
